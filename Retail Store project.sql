...
Select transaction_id, category, quantity, sale_date from retail_sales where category = 'Clothing' and quantity>=4 
...
SELECT year,month,avg_sale FROM (SELECT EXTRACT(YEAR FROM sale_date) as year, EXTRACT(MONTH FROM sale_date) as month,
AVG(total_sale) as avg_sale, RANK() OVER(PARTITION BY EXTRACT(YEAR FROM sale_date) ORDER BY AVG(total_sale) DESC) as ranks 
